...
class OldAccount(Account):
    ...
    def get_xpubkeys(self, for_change, n):
        s = ''.join(map(lambda x: pesetacoin.int_to_hex(x,2), (for_change, n)))
    ...
    def parse_xpubkey(self, x_pubkey):
        ...
        s = []
        while dd:
            n = int(pesetacoin.rev_hex(dd[0:4]), 16)
            dd = dd[4:]
            s.append(n)
        assert len(s) == 2
        return mpk, s
...
class BIP32_Account(Account):
    ...
    def get_xpubkeys(self, for_change, n):
        ...
        s = ''.join(map(lambda x: pesetacoin.int_to_hex(x,2), (for_change,n)))
        xpubs = self.get_master_pubkeys()
        return map(lambda xpub: 'ff' + pesetacoin.DecodeBase58Check(xpub).encode('hex') + s, xpubs)
    ...
    def parse_xpubkey(self, pubkey):
        ...
        pk = pk[1:]
        xkey = pesetacoin.EncodeBase58Check(pk[0:78])
        dd = pk[78:]
        s = []
        while dd:
            n = int( pesetacoin.rev_hex(dd[0:2].encode('hex')), 16)
            dd = dd[2:]
            s.append(n)
        assert len(s) == 2
        return xkey, s
