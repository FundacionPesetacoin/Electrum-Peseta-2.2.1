...
from electrum_peseta import DEFAULT_SERVERS, DEFAULT_PORTS
...
class NetworkDialog(QDialog):
    def __init__(self, network, config, parent):
        ...
        hbox.addStretch(50)
        msg = _("Electrum-PESETA sends your wallet addresses to a single server, in order to receive your transaction history.") + "\n\n" \
            + _("In addition, Electrum-PESETA connects to several nodes in order to download block headers and find out the longest blockchain.") + " " \
        ...
        msg = _("If auto-connect is enabled, Electrum-PESETA will always use a server that is on the longest blockchain.") + " " \
            + _("If it is disabled, Electrum-PESETA will warn you if your server is lagging.")
