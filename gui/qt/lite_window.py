...
try:
    ...
    print "You need to have PyQT installed to run Electrum-PESETA in graphical mode."
...
from electrum_peseta.util import get_resource_path as rsrc
from electrum_peseta.pesetacoin import is_valid
...
import re
import time
from electrum_peseta.wallet import Wallet, WalletStorage
import webbrowser
...
from electrum_peseta.version import ELECTRUM_VERSION as electrum_version
from electrum_peseta.util import format_satoshis, age
...
pesetacoin = lambda v: v * 100000000
...
class MiniWindow(QDialog):
    ...
    def __init__(self, actuator, expand_callback, config):
        ...
        self.address_input.setPlaceholderText(_("Enter a Pesetacoin address or contact"))
    ...
    def set_balances(self, btc_balance):
        """Set the pesetacoin balance and update the amount label accordingly."""
        ...
        self.setWindowTitle("Electrum-PESETA %s - %s %s" % (electrum_version, amount, unit))

    def amount_input_changed(self, amount_text):
        """Update the number of pesetacoins displayed."""
    ...
    def check_button_status(self):
        """Check that the pesetacoin address is valid and that something
    ...
    def the_website(self):
        webbrowser.open("http://electrum.pesetacoin.info")
...
class BalanceLabel(QLabel):
    ...
    def set_balance_text(self, amount, unit, quote_text):
        """Set the amount of pesetacoins in the gui."""
        if self.state == self.SHOW_CONNECTING:
...
class ReceivePopup(QDialog):
    ...
    def setup(self, address):
        label = QLabel(_("Copied your Pesetacoin address to the clipboard!"))
        ...
        self.setWindowTitle("Electrum-PESETA - " + _("Receive Pesetacoin payment"))
...
class MiniActuator:
    ...
    sending/receiving pesetacoins."""


    def __init__(self, main_window):
        """Retrieve the gui theme used in previous session."""
        self.g = main_window
        self.theme_name = self.g.config.get('litegui_theme','Peseta')
        self.theme_name = 'Peseta'
    ...
    def waiting_dialog(self, f):
        ...
        w = QDialog()
        w.resize(200, 70)
        w.setWindowTitle('Electrum-PESETA')
    ...
    def send(self, address, amount, parent_window):
        """Send pesetacoins to the target address."""
        ...
        fee = 0
        # 0.1 PTC = 10000000
        if amount < pesetacoin(1) / 10:
            # 0.001 PTC
            fee = pesetacoin(1) / 1000
