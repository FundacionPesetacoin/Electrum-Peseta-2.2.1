...
from electrum_peseta.util import print_error, InvalidPassword
from electrum_peseta.pesetacoin import is_valid
from electrum_peseta import WalletStorage, Wallet

Gdk.threads_init()
APP_NAME = "Electrum-PESETA"
import platform
MONOSPACE_FONT = 'Lucida Console' if platform.system() == 'Windows' else 'monospace'

from electrum_peseta.util import format_satoshis, parse_URI
from electrum_peseta.network import DEFAULT_SERVERS
...
def restore_create_dialog():
    ...
    dialog = Gtk.Dialog("electrum-peseta", parent=None,
                        flags=Gtk.DialogFlags.MODAL,
...
def run_network_dialog( network, parent ):
    ...
    add_help_button(host_box, 'The name, port number and protocol of your Electrum-PESETA server, separated by a colon. Example: "ecdsa.org:50002:s". Some servers allow you to connect through http (port 80) or https (port 443)')
    host_box.show()
...
class ElectrumWindow:
    ...
    def __init__(self, wallet, config, network):
        ...
        title = 'Electrum-PESETA ' + self.wallet.electrum_version + '  -  ' + self.config.path
    ...
    def do_send(self, w, data):
        ...
        if not is_valid(to_address):
            self.show_message( "invalid pesetacoin address:\n"+to_address)
            return
        ...
        #@todo pesetacoin electrum-ltc modifies this...
